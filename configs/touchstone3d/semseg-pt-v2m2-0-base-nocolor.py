...
# dataset settings
dataset_type = "Touchstone3DDataset"
data_root = "data/touchstone3d"

data = dict(
    num_classes=18,
    ignore_index=-1,
    names=["WALL","FLOOR","CEILING","DOOR","WINDOW","CABINET","SHELF","ISLAND",
            "CHAIR","PILLOW","COUCH","DINING_TABLE","DESK","SIDE_TABLE",
            "COFFEE_TABLE","WATER_CLOSET","BATHTUB","BED"],
    train=dict(
        type=dataset_type,
        split=("House1", "House2", "House3", "House4", "House5",
        ...
        transform=[
            ...
            dict(type="GridSample", grid_size=0.04, hash_type="fnv", mode="train",
                 keys=("coord", "segment"), return_discrete_coord=True),
            dict(type="SphereCrop", point_max=80000, mode="random"),
            dict(type="CenterShift", apply_z=False),
            dict(type="NormalizeColor"),
            # dict(type="ShufflePoint"),
            dict(type="ToTensor"),
            dict(type="Collect", keys=("coord", "discrete_coord", "segment"), feat_keys=["coord"])
        ],
        test_mode=False
    ),
    val=dict(
        type=dataset_type,
        split=("House21","House22","House23","House24","House25","House26"),
        data_root=data_root,
        transform=[
            dict(type="CenterShift", apply_z=True),
    ...
    test=dict(
        type=dataset_type,
        split=("House27","House28","House29","House30","House31","House32"),
        data_root=data_root,
        transform=[
            dict(type="CenterShift", apply_z=True),
            #dict(type="NormalizeColor")
